    ...
    <div className={`mb-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 ${wide}`}>
      {cells.map((c, i) => (
        <div key={c.label} style={{ '--i': i } as React.CSSProperties}
             className="anim-fade-up lift glass-card flex items-stretch gap-2.5 rounded-lg border border-line py-2 pr-2.5 shadow-sm">
...
function ExecTile({ k, index, chart, notes }: {
  ...
  return (
    <div style={{ '--i': index } as React.CSSProperties}
         className={`anim-fade-up lift glass-card flex flex-col rounded-lg border p-3 shadow-sm ${
