...
export function Dialog({ open, onClose, title, sub, children, wide }: {
  ...
  if (!open) return null
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-ink/25 p-4 pt-[10vh] backdrop-blur-[2px]"
         onClick={onClose}>
      <div ref={ref} role="dialog" aria-modal="true" aria-label={title}
           onClick={(e) => e.stopPropagation()}
           className={`w-full rounded-lg border border-line bg-surface shadow-xl ${wide ? 'max-w-3xl' : 'max-w-lg'}`}>
