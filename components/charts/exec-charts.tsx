    ...
    <div>
      <ul className="space-y-1.5">
        {rows.map((r, i) => {
          ...
          return (
            <li key={r.label} className="grid grid-cols-[minmax(6.5rem,1.1fr)_1fr_auto] items-center gap-2">
              <span className="min-w-0">
                <span className="block truncate text-[11px] text-ink-2" title={r.label}>{r.label}</span>
                {r.sub && <span className="mono block truncate text-[9.5px] text-ink-3">{r.sub}</span>}
              </span>
      ...
      <ul className="mt-2 flex flex-wrap gap-x-3.5 gap-y-1">
        ...
        {more && <li className="mono text-[10px] text-ink-3">{more}</li>}
      </ul>
    </div>
  )
}
 ...
 * the work on its own.
 */
export function Donut({ segments, centre, centreSub, foot }: {
  segments: { label: string; value: number; color: string; hatched?: boolean }[]
  centre: string; centreSub?: string
  /** one line under the legend — usually what the segments are worth, or which is assumed */
  foot?: string
}) {
  const uid = useId().replace(/[^a-zA-Z0-9]/g, '')
  ...
  return (
    <div className="flex flex-wrap items-center gap-4">
      <svg viewBox="0 0 100 100" className="h-32 w-32 shrink-0" role="img"
      ...
      {foot && <p className="w-full text-[10.5px] leading-snug text-ink-3">{foot}</p>}
    </div>
  )
}
...
/**
 * A dial, for a single rate a reader judges against a limit rather than reads
 * to two decimals. The flat meter was right about the data and wrong about the
 * glance: at a quarter-tile wide it reads as a progress bar, and progress bars
 * imply "filling up is good", which is false for a return rate.
 *
 * Everything is on the arc: the fill, the target as a notch, and the figure in
 * the middle. Nothing needs a second look to place the number on its scale.
 */
export function Gauge({ value, max, target, targetLabel, tone = 'accent', hatched, unit = '%', dp = 1, sub }: {
  value: number; max: number
  target?: number; targetLabel?: string
  tone?: keyof typeof TONE; hatched?: boolean; unit?: string; dp?: number
  /** one line under the dial — what the number is measured against */
  sub?: string
}) {
  const uid = useId().replace(/[^a-zA-Z0-9]/g, '')
  const col = TONE[tone]
  const R = 38, CX = 50, CY = 46
  const L = Math.PI * R                      // a half turn, so the whole arc is the scale
  const f = Math.max(0, Math.min(1, value / max))
  const arc = `M ${CX - R} ${CY} A ${R} ${R} 0 0 1 ${CX + R} ${CY}`
  const at = (t: number, r1: number, r2: number) => {
    const a = Math.PI - Math.PI * Math.max(0, Math.min(1, t))
    return { x1: CX + r1 * Math.cos(a), y1: CY - r1 * Math.sin(a),
             x2: CX + r2 * Math.cos(a), y2: CY - r2 * Math.sin(a) }
  }
  const tick = target != null ? at(target / max, R - 7, R + 7) : null
  return (
    <div>
      <svg viewBox="0 0 100 58" className="mx-auto block w-full max-w-[172px]" role="img"
        ...
        <text x="50" y="43" textAnchor="middle" fontSize="15" fontWeight="600" fill="var(--ink)"
              fontFamily="var(--font-plex-sans)">{num(value, dp)}<tspan fontSize="8" fill="var(--ink-3)">{unit}</tspan></text>
        <text x={CX - R - 1} y="57.5" textAnchor="middle" fontSize="6.5" fill="var(--ink-3)"
              fontFamily="var(--font-plex-mono)">0</text>
        <text x={CX + R + 1} y="57.5" textAnchor="middle" fontSize="6.5" fill="var(--ink-3)"
              fontFamily="var(--font-plex-mono)">{num(max, 0)}{unit.trim() === '%' ? '%' : ''}</text>
      </svg>
      {(target != null || sub) && (
        <p className="mono -mt-0.5 flex flex-wrap items-center justify-center gap-x-2 text-[10px] text-ink-3">
          {target != null && (
            <span className="flex items-center gap-1">
              <span aria-hidden className="inline-block h-2.5 w-px bg-ink" />
              {targetLabel ?? `target ${num(target, 1)}${unit}`}
            </span>
          )}
          {sub && <span>{sub}</span>}
        </p>
...
/**
 * The two or three rows behind a chart that a reader would otherwise have to
 * open the derivation to see — which receipt was late, which job stops, which
 * PO lands after the material runs out.
 *
 * It exists because a chart of five cells and a legend is short, and a tile
 * that ends early in a row of four leaves a hole. Filling the hole with the
 * exceptions is better than filling it with air, and better than padding.
 */
export function Evidence({ title, rows }: {
  title: string; rows: { k: string; v: string; tone?: keyof typeof TONE }[]
}) {
  if (!rows.length) return null
  return (
    <div className="mt-2 border-t border-line-soft pt-1.5">
      <p className="mono text-[9.5px] uppercase tracking-wide text-ink-3">{title}</p>
      <ul className="mt-1 space-y-0.5">
        {rows.map((r) => (
          <li key={r.k} className="flex items-baseline gap-1.5 text-[10.5px] leading-snug">
            {r.tone && <span aria-hidden className="size-1.5 shrink-0 translate-y-px rounded-full"
                             style={{ background: TONE[r.tone] }} />}
            <span className="mono shrink-0 text-ink-2">{r.k}</span>
            <span className="min-w-0 text-ink-3">{r.v}</span>
          </li>
        ))}
      </ul>
    </div>
